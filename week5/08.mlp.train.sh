...
case "$nnet_type" in
  ...
  lstm)
    utility/make_lstm_proto.py $proto_opts \
       $num_feat $num_target --num-cells=400 --num-layers=$depth --num-recurrent=256 >$nnet_proto || exit 1
...
echo ""
echo "We use $nnet_type neural network, and $train_tool as our training tool."
